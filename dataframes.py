...
X4_max = df['X4 number of convenience stores'].max() # Put the value of the max value of X4 here -- 1.3 of Gradescope test
...
mean_X3_fill =  df_fill['X3 distance to the nearest MRT station'].mean() # Put # Put the mean value calculated here -- 1.8 of Gradescope tests
...
New_count = df_age_classify['Age Class'].value_counts().get('New', 0) # Count of houses classified as 'New' -- 1.10 of Gradescope tests
Middle_count = df_age_classify['Age Class'].value_counts().get('Middle', 0) # Count of houses classified as 'Middle' -- 1.11 of Gradescope tests
Old_count = df_age_classify['Age Class'].value_counts().get('Old', 0) # Count of houses classified as 'Old' -- 1.12 of Gradescope tests
...
reindex_df = dataframe.reset_index(drop=True).set_index('No') # YOUR CODE HERE
...
#print(grouped_age_price_df.head())
#print(grouped_age_price_df.describe())
...
mean_val_conv_7 = round(grouped_age_price_df[grouped_age_price_df['X4 number of convenience stores'] == 7]['Y house price of unit area'].mean(), 2) # Put the value here -- 1.17 of Gradescope tests
...
grouped_age_price_df.plot.bar(x='X4 number of convenience stores', y='Y house price of unit area', legend=False)
